...
class ApiService {
  ...
  Future<List<dynamic>?> getPendencias({String? cnpj}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('jwt_token');
      ...
      final uri = Uri.parse(
        '$baseUrl/pendencias',
      ).replace(queryParameters: cnpj != null ? {'cnpj': cnpj} : null);
      ...
      throw Exception('Falha ao carregar pendencias: ${response.statusCode}');
    } catch (e) {
      print("Erro no getPendencias: $e");
      return null;
    }
  }

  Future<bool> criarPendencia(Map<String, dynamic> pendencia) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('jwt_token');

      final response = await http.post(
        Uri.parse('$baseUrl/pendencias'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true',
        },
        body: jsonEncode(pendencia),
      );
  ...
  Future<bool> atualizarPendencia(int id, Map<String, dynamic> dados) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('jwt_token');

      final response = await http.put(
        Uri.parse('$baseUrl/pendencias?id=$id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true',
      ...
      print("Erro ao atualizar pendência: $e");
      return false;
    }
  }

  Future<bool> deletarPendencia(int id) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('jwt_token');
      ...
      print("Erro ao deletar pendência: $e");
      return false;
    }
  }
