...
Response _handleGet(String? id) {
  try {
    ...
    final resultados = (id != null)
        ? dbHelper.db.select(
            '''SELECT tarefas.*, usuarios.nome AS assignee_nome
           FROM tarefas
           LEFT JOIN usuarios ON usuarios.username = tarefas.assignee
           WHERE tarefas.id = ?''',
            [int.tryParse(id)],
          )
        : dbHelper.db.select('''
    ...
    final lista = resultados.map((row) {
      ...
      return {
        ...
        'assignee_nome': row['assignee_nome'],
        'cnpj': row['cnpj'],
...
Future<Response> _handlePost(RequestContext context) async {
  ...
  dbHelper.db.execute(
    'INSERT INTO tarefas (cliente, assignee, cnpj, tipo, prazo, status, observacao, logs, urgente) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [
      body['cliente'],
      body['assignee'],
      body['cnpj'],
...
Future<Response> _handlePut(RequestContext context, String? id) async {
  ...
  dbHelper.db.execute(
    'UPDATE tarefas SET cliente = ?, cnpj = ?, assignee = ?, tipo = ?, prazo = ?, status = ?, observacao = ?, logs = ?, urgente = ? WHERE id = ?',
    [
      body['cliente'] ?? row['cliente'],
      body['cnpj'] ?? row['cnpj'],
      body['assignee'] ?? row['assignee'],
